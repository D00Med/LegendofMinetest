...
local function register_clawshot(name, desc, texture, texture2, speed, accel, timeout, time2)
		...
		local objs = minetest.get_objects_inside_radius(pos, 2)	
			for k, obj in pairs(objs) do
				if obj:get_luaentity() ~= nil then
					if obj:get_luaentity().name ~= self.object:get_luaentity().name and obj:get_luaentity().name ~= "__builtin:item" then
	...
	on_use = function(item, placer, pointed_thing)
			...
			local obj = minetest.env:add_entity({x=playerpos.x+0+dir.x,y=playerpos.y+1.4+dir.y,z=playerpos.z+0+dir.z}, "clawshot:"..name.."_chain")
