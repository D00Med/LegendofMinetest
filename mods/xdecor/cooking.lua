...
xdecor.register("cauldron_potion", {
	groups = {cracky=2, oddly_breakable_by_hand=1, not_in_creative_inventory=1},
	on_rotate = screwdriver.rotate_simple,
	drop = "xdecor:cauldron_empty",
	infotext = "Cauldron (active) - Use a bucket to collect the potion",
	damage_per_second = 2,
	tiles = {
		{ name = "xdecor_cauldron_top_anim_potion.png",
			animation = {type="vertical_frames", length=3.0} },
		"xdecor_cauldron_sides.png"
	},
	collision_box = cauldron_cbox,
	on_rightclick = function(pos, node, clicker, itemstack, _)
		...
		if clicker:get_wielded_item():get_name() == "bucket:bucket_empty" then
			if inv:room_for_item("main", "xdecor:bucket_potion 1") then
				itemstack:take_item()
				inv:add_item("main", "xdecor:bucket_potion 1")
				minetest.set_node(pos, {name="xdecor:cauldron_empty", param2=node.param2})
			else
				minetest.chat_send_player(clicker:get_player_name(),
						"No room in your inventory to add a bucket of potion!")
			end
			return itemstack
		end
	end
...
minetest.register_abm({
	...
	action = function(pos, node, _, _)
		...
		for _, obj in pairs(objs) do
			if obj and obj:get_luaentity() then
				local itemstring = obj:get_luaentity().itemstring:match("([%w_:]+)%s")
				if itemstring and not minetest.serialize(magic):find(itemstring) and
						(itemstring:find("magicpowder") or itemstring:find("diamond")) then	
					magic[#magic+1] = itemstring
				end
			end
		end
		...
		if #magic >= 1 then
			...
			minetest.set_node(pos, {name="xdecor:cauldron_potion", param2=node.param2})
		end
